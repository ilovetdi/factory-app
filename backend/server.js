...
async function init(){
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS settings(id SERIAL PRIMARY KEY, external_url TEXT);`);
  await pool.query(`CREATE TABLE IF NOT EXISTS users(id SERIAL PRIMARY KEY, username TEXT, password TEXT, role TEXT);`);
...
app.post("/upload", upload.single("file"), (req,res)=>{
  ...
  let clean = filePath.replace("uploads/","");
  let url = externalBase ? externalBase + "/" + clean : "/uploads/" + clean;
  res.json({ path: url });
});
